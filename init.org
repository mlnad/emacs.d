...
| Provider    | Args                           | Env               |
|-------------+--------------------------------+-------------------|
| deepseek-v3 | --model deepseek/deepseek-chat | DEEPSEEK_API_KEY  |
| deepseek-r1 | --model r1                     | DEEPSEEK_API_KEY  |
| claude      | --model anthropic/claude-3-5   | ANTHROPIC_API_KEY |
| gpt4        | --model openai/gpt-4           | OPENAI_API_KEY    |
...
(defvar moyu/ai-provider-config 
  (cl-loop for row in providers
           ...
           return `(:args ,(split-string (cadr row))
                          :env ,(caddr row)))
...
(use-package aider
  ...
  (setq aider-args (plist-get moyu/ai-provider-config :args))
...
** Font & Theme
#+name: font-theme
| Font set     | family            | size | weight | slant  |
|--------------+-------------------+------+--------+--------|
| default-font | Consolas          |   14 | normal | normal |
| serif-font   | Noto Serif CJK SC |   14 | normal | normal |
| symbol       | 'nil              |      |        |        |
| mathematical | 'nil              |      |        |        |
| unicode      | Weather Icons     |      |        |        |
| unicode      | github-octicons   |      |        |        |
| unicode      | FontAwesome       |      |        |        |

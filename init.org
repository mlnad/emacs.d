...
3. Completion: Implements advanced completion frameworks (e.g.,
   Vertico, Corfu) and project management tools.

4. Programming: Provides language-specific configurations for Lisp, Rust,
   Python, and more, with support for projects, LSP and debugging.
...
;;; early-init.el --- Framework -*- lexical-binding: t; -*-
...
(defvar moyu/cache-directory (expand-file-name ".cache/" user-emacs-directory))
...
#+begin_src emacs-lisp :tangle yes
(defvar moyu/available-package-mirrors
...
(defvar moyu/used-package-mirror
  (let ((mirror (alist-get moyu/package-mirror moyu/available-package-mirrors)))
    (unless mirror
...
** Enviroment
The environment is kept in an ordinary Emacs Lisp file,
=~/.emacs.d/.cache/env.el=, created once by =moyue install= from the login
shell and editable by hand afterwards. Load it on every startup so that
Emacs launched from a desktop launcher inherits the shell environment
(=PATH=, =GOPATH=, =NVM_DIR=, …).
#+begin_src emacs-lisp :tangle yes
;; `moyu/env-load' applies `.cache/env.el' (if present) to this session.
(moyu/env-load 'noerror)
#+end_src
...
(unless (eq system-type 'windows-nt)
  (setq tramp-default-method "ssh"
        ...
        tramp-auto-save-directory (moyu/cache-it "tramp-autosave/")
        tramp-persistency-file-name (moyu/cache-it "tramp")
...
(use-package savehist
  ...
  (setq savehist-save-minibuffer-history t
        ...
        savehist-additional-variables '(mark-ring
                                        ...
                                        kill-ring))
  ;; `evil-jumps-history' holds marker chains that share structure.  savehist
  ;; writes with `print-circle' bound to t, so those entries are emitted as
  ;; `#N=' definitions plus `#N#' references, while its per-element write/read
  ;; check can then discard the definitions.  The result is a savehist file
  ;; with dangling `#N#' refs that fails to load on the next startup
  ;; ("Invalid read syntax: \"#1#\""), losing the whole history.  Exclude it
  ;; here; `evil-jumps' keeps its own state.
...
(use-package bookmark
  :custom
  (bookmark-default-file (moyu/cache-it "bookmarks")))
#+end_src
...
(use-package treesit
  ...
  :init
  ;; Define grammar sources for auto-download. Emacs 29+
  ...
  ;; Remap classic modes to their tree-sitter equivalents (Emacs 29+).
  ;; This ensures ts-modes are activated automatically for matching files.
  ...
  :config
  ;; Safely ensure grammars are available (they can be auto-installed on first use).
  ;; treesit-install-language-grammar requires compiler tools (gcc/clang)
  (when (fboundp 'treesit-install-language-grammar)
    (dolist (lang '(bash c cpp cmake css dockerfile python json rust yaml))
      ;; Only attempt install if grammar not yet loaded
...
| deepseek-r1 | r1                     | DEEPSEEK_API_KEY  |
| claude      | anthropic/claude-3-5   | ANTHROPIC_API_KEY |
| gpt4        | openai/gpt-4           | OPENAI_API_KEY    |

#+begin_src emacs-lisp :var providers=ai-providers :tangle yes
(defvar moyu/ai-provider-config 
  (cl-loop for row in providers
           when (string= (car row) moyu/ai-provider)
           return `(:model ,(cadr row) :env ,(caddr row)))
...
(use-package aidermacs
  ...
  :config
  (setq aidermacs-default-model (plist-get moyu/ai-provider-config :model))
  (when moyu/ai-api-key
    (setenv (plist-get moyu/ai-provider-config :env)
            moyu/ai-api-key)))
#+end_src
...
(use-package dape
  ...
  (dape-default-breakpoints-file (moyu/cache-it "dape-breakpoints"))
  :config
...
(use-package ielm
  :custom
  (ielm-history-file-name (moyu/cache-it "ielm-history.eld")))
#+end_src
...
;; `lispy' ships with an empty `Package-Requires', so `:ensure t' above pulls
;; in no dependency at all.  Declare them explicitly, otherwise a fresh
;; `moyue install' produces a lispy that cannot be loaded
;; ("Cannot open load file: iedit" / "swiper").  `ivy' and `lv' arrive as
;; dependencies of `swiper' and `hydra'.  `ace-window' is required lazily.
(use-package zoutline :ensure t :defer t)
...
(use-package org
  ...
  ;; Keep Org's persistent cache inside our own cache directory.  Org reads and
  ;; writes it while setting up `org-mode'; if that I/O signals an error (cache
  ;; dir missing/unwritable, or a stale index from another Org version), the
  ;; mode specification is aborted and the buffer is left without font-lock and
  ;; without org-modern/org-indent -- i.e. org loses all its colors while other
  ;; major modes keep theirs.
  (setq org-persist-directory (moyu/cache-it "org/org-persist/"))
  ;; org babels
  ...
  ;; org agenda
  (setq-default org-agenda-files (list (expand-file-name "agendas" moyu/notes-dir))
...
(use-package rime
  ...
  :config
  (let ((rime-user-file (expand-file-name "user.yaml" rime-user-data-dir)))
    (unless (file-exists-p rime-user-file)
      (unless (file-exists-p rime-user-data-dir)
        (mkdir rime-user-data-dir))
...
| Face              | Family         | Size |
|-------------------+----------------+------|
| default           | JetBrains Mono |   16 |
| fixed-pitch       | JetBrains Mono |   16 |
| fixed-pitch-serif | JetBrains Mono |   16 |
| cjk               | LXGW WenKai    |   16 |
#+begin_src emacs-lisp :tangle yes :var face-fonts=face-fonts
(defvar moyu/face-fonts face-fonts
  "Font configuration table.
Each row is (FACE FAMILY SIZE).  Special FACE values:
  cjk  — CJK font family for the default fontset")

(defun moyu/apply-face-fonts ()
  (when (display-graphic-p)
    (dolist (row moyu/face-fonts)
      (cl-destructuring-bind (face family size) row
        (cond
         ((string= face "cjk")
          ;; Deliberately leave :weight and :slant unspecified so Emacs can
          ;; select the corresponding LXGW WenKai variant for the active face.
          (dolist (cs '(han cjk-misc bopomofo kana))
            (set-fontset-font t cs (font-spec :family family :size size))))
         (t
          (set-face-attribute (read face) nil
                              :width 'normal :weight 'normal :slant 'normal
                              :font (font-spec :family family :size size))))))))

(add-hook 'window-setup-hook #'moyu/apply-face-fonts)
...
*** Fontset fonts
#+name: fontset-fonts
| Char set | family          | weight | slant  |
|----------+-----------------+--------+--------|
| unicode  | Weather Icons   | normal | normal |
| unicode  | github-octicons | normal | normal |
| unicode  | FontAwesome     | normal | normal |
| unicode  | all-the-icons   | normal | normal |
| unicode  | file-icons      | normal | normal |
| unicode  | Material Icons  | normal | normal |

#+begin_src emacs-lisp :tangle yes :var fontset-fonts=fontset-fonts
(defvar moyu/fontset-fonts
  fontset-fonts)

(defun moyu/apply-fontset-fonts ()
  (mapc #'(lambda (row)
            (cl-destructuring-bind (char-set family weight slant) row
              (set-fontset-font t (read char-set)
...
(add-hook 'after-init-hook #'moyu/apply-fontset-fonts)
(add-hook 'server-after-make-frame-hook #'moyu/apply-fontset-fonts)
#+end_src
...
(defun moyu/apply-theme ()
  (when (and moyu/theme (display-graphic-p) (not (custom-theme-enabled-p moyu/theme)))
    (mapc #'disable-theme custom-enabled-themes)
    (load-theme moyu/theme t)))

(add-hook 'after-init-hook #'moyu/apply-theme)
(add-hook 'server-after-make-frame-hook #'moyu/apply-theme)
#+end_src

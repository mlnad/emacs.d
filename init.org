...
(defvar configs/lsp-client 'eglot)
...
(unless (file-exists-p configs/userconfig-file)
  (with-temp-file configs/userconfig-file
    (setq-local coding-system-for-write 'utf-8)
    (goto-char (point-min))
    (insert
     ";; -*- mode: emacs-lisp -*-

(setq configs/package-mirror emacs-tuna-package-mirror
      configs/default-font '(\"Source Code Pro\"
                             :size 15
                             :weight normal
                             :width normal))")))
...
(use-package buttercup
  ...
  :init
  (add-to-list 'popper-buffers "^\\*Buttercup\\*$")
#+end_src
...
(use-package rustic
  ...
  :mode ("\\.rs$" . rustic-mode)
  :init
  (add-to-list 'popper-buffers "^\\*cargo-.*\\*$")
  :config
  ...
  ;;      to reconfigure them.
  (setq rustic-lsp-client configs/lsp-client))
#+end_src
...
(use-package python
  ...
  :init
  (add-to-list 'popper-buffers "^\\*Python"))
#+end_src

** TODO C/C++
use =cc-mode= and =cmake-mode=
...
(use-package geiser
  ...
  :commands run-geiser
  :init
  (add-to-list 'popper-buffers "^\\*[gG]eiser \\(dbg\\|xref\\|messages\\)\\*$")
  (add-to-list 'popper-buffers "^\\*Geiser documentation\\*$")
  (add-to-list 'popper-buffers "^\\*Geiser .+ REPL"))
#+end_src
...
(use-package tex
  :ensure auctex
  :init
  (add-to-list 'popper-buffers " output\\*$")
  (add-to-list 'popper-buffers "^\\*TeX \\(?:Help\\|errors\\)"))
...
(use-package magit
  :ensure t
  :init
  (add-to-list 'popper-buffers "^\\(?:\\*magit\\|magit:\\| \\*transient\\*\\)"))
...
(use-package rime
  ...
  (add-hook 'after-init-hook
            (lambda ()
              (let ((rime-user-file (concat configs/rime-data-dir "user.yaml")))
...
(add-to-list 'popper-buffers "\\.*eshell.*\\*$")
(add-to-list 'popper-buffers 'eshell-mode)
(add-to-list 'popper-buffers "\\.*-shell.*\\*$")
(add-to-list 'popper-buffers 'shell-mode)
#+end_src
...
(use-package popper
  ...
  :bind (("C-`"    . popper-toggle)
         ("M-'"    . popper-cycle))
  :init
  (setq popper-reference-buffers popper-buffers)
  (popper-mode +1)
  (popper-echo-mode +1)
  :config

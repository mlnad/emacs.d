...
This file serves as the central configuration for Emacs, a highly customized
   Emacs setup designed for efficiency, flexibility, and productivity. It
   leverages Org-mode's powerful tangling capabilities to generate Emacs Lisp
   code, ensuring a clean and maintainable configuration structure. The
   configuration is modular, with sections dedicated to initialization, editor
   settings, completion frameworks, programming environments, and user interface
   enhancements.
...
| userconfig-file  | (expand-file-name "userconfig" moyu/cache-directory) |
...
(use-package rustic
  ...
  (add-hook 'rustic-mode-local-vars-hook
            (lambda () (setq rustic-lsp-client moyu/lsp-client))))
#+end_src
...
(use-package python
  ...
  :init
  (setq python-environment-directory moyu/cache-directory
